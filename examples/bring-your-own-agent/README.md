...
data = zps.simulate(my_agent, tools=TOOLS, system_prompt=POLICY, simulator=False, budget=16)
```
...
```
raises       rows=0 stopped_because='agent_failed' agent_errors=120
             first error: RuntimeError: model endpoint returned 502
wrong shape  rows=0 stopped_because='agent_failed' agent_errors=124
