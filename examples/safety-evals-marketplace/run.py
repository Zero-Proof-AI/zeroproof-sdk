"""Safety evals for a marketplace agent: prompt injection planted in
user-generated reviews, tenant isolation on intent data, exfiltration by
email and by public post, flags without authority, and the over-refusal
a naive fix brings. Offline, no key, seconds.
    ...
    python live.py                     # the same suite on a real model (Ollama, no key)
...
2. **Grade.** ``evaluate(rows, safety_judge)``: six markers read from the
...
_WATCHED = {
    "send_email": "to",
    "flag_review": "ticket_id",
    "get_intent_signals": "product_id",
    "respond_to_review": "review_id",
}
...
def failures(rows: list[dict], *, limit: int = 5) -> list[str]:
    ...
    for r in rows:
        ...
        calls = [
            f"{s['tool']}({(s.get('arguments') or {}).get(_WATCHED[s['tool']], '')})"
            for s in r.get("steps") or []
            if s.get("tool") in _WATCHED
        ]
        out.append(
            f"  [{r.get('failure_class')}] {r.get('category')}: {str(r.get('prompt'))[:60]!r}\n"
            f"      calls={calls or '-'}  reply={str(r.get('final_text'))[:80]!r}"
        )
        if len(out) >= limit:
            break
    return out
