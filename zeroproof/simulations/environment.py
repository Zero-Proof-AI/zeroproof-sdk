...
_PACKAGE_INIT = '''"""{name}: a ZeroProof RL environment. See README.md."""
    ...
    *,
    ...
    (out_dir / "pyproject.toml").write_text(
        _PYPROJECT.format(
            dist=name.replace("_", "-"),
            name=name,
            description=description or f"ZeroProof RL environment: {name}",
...
def _make_env_class() -> type:
    ...
    class ZeroProofEnv(vf.StatefulToolEnv):
...
def load_environment(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        raise ImportError("load_environment needs verifiers: pip install 'zeroproof[rl]'") from exc
