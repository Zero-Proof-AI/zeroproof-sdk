...
def make_candidate_generator(
    ...
    *,
    ...
    def generate(dataset: Any = None, index: int | None = None) -> list[str]:
        ...
        # Cards, not sentences. This arm picks which cells of the covering
        # grid this round should fill and what each cell's world looks
        # like; the model writes the words. Nothing here is a phrasing:
        # canned openers made every customer's dataset share the same
        # sentences, which is the opposite of training data.
        generate.picked_regions = list(picked)
        generate.steered_regions = set(steered)
        generate.cell_faults = {
            region["id"]: fault_plan_for_region(region)
            for region in picked
            if fault_plan_for_region(region)
        }
        del open_budget
        return []
