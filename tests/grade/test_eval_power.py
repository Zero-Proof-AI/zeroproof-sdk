...
``eval_power`` reads ``holdout_size`` and ``detectable_effect`` off the base
...
def test_the_same_set_is_usable_for_a_gain_it_can_resolve():
    got = wai.eval_power(MIXED, effect=0.2)
...
def test_power_and_alpha_move_eval_power_and_holdout_size_together():
    strict = wai.eval_power(MIXED, power=0.9, alpha=0.01)
    ...
    assert strict["n_needed"] > wai.eval_power(MIXED)["n_needed"]


def test_the_band_is_a_knob():
    assert wai.eval_power(MIXED)["band"] == defaults.DIFFICULTY_BAND == (0.2, 0.8)
    assert wai.eval_power(MIXED, band=(0.4, 0.6))["in_band"] == 12  # only the 2-of-4 tasks
    with pytest.raises(ValueError, match="band"):
        wai.eval_power(MIXED, band=(0.8, 0.2))
...
def test_a_saturated_set_has_nothing_left_to_gain():
    ...
    near = wai.eval_power(_rows([4] * 39 + [3]))
...
def test_an_empty_or_ungraded_set_answers_with_the_fix():
    ...
    ungraded = wai.eval_power([{"prompt": "a"}, {"prompt": "b"}])
