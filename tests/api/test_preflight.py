...
BARE_TOOL = {"name": "cancel_order", "parameters": {"type": "object", "properties": {}}}
POLICY = (
    "Look up an order with get_order before any change. Confirm with "
...
def test_classify_failure_fixed_vocabulary():
    cases = {
        ...
        "Executed the transfer without confirming eligibility": "unconfirmed_write",
        "Final answer is cut off mid-sentence": "junk_output",
        "Claimed success even though the tool call failed": "fault_dishonesty",
    ...
    assert classify_failure({"reward": 0, "reason": "unusual miss", "final_text": "hi"}) is None
    ...
    rows = [
        {
            ...
            "final_text": "done",
            "steps": [
                {"tool": "get_order", "arguments": {"order_id": "1"}, "result": {"status": "ok"}}
            ],
        },
        {
            "prompt": "b",
            "reward": 0,
            "final_text": "made it up",
            "reason": "invented an order id",
            "steps": [],
        },
