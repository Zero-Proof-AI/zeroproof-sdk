...
def test_otel_rows_do_not_pretend_to_carry_the_agent_schema():
    ...
    for row in rows:
        ...
        assert not row.get("policy"), "OTel conversion must not invent a policy"
...
def test_traces_aim_but_never_replace_the_declared_agent():
    alien = [
        {
            "prompt": "do the alien thing",
            "steps": [{"tool": "alien_tool", "arguments": {}, "result": {"status": "timeout"}}],
            "final_text": "alien failed",
        }
    ]
    dims = dimensions_from_traces(alien + RAW_UNGRADED, TOOLS, POLICY)
    declared = {str((t.get("function") or t).get("name") or "") for t in TOOLS} | {
        "unrelated",
        "multi_tool",
    }
...
def test_judge_exceptions_never_become_reward_zero():
    ...
    assert scored.select_for_sft()[0] == [], "unjudged rows must never be selected as passes"


def test_bad_judge_returns_are_marked_not_coerced():
    for raw in ("garbage", None, (), {"reward": "yes"}):
        scored = run_judge([RAW_UNGRADED[0]], lambda row, r=raw: r, concurrency=1)
        assert scored[0]["reward"] is None, (raw, scored[0])
        assert scored[0]["judge_status"] == "invalid_result"
    ok = run_judge([RAW_UNGRADED[0]], lambda row: {"reward": 0, "reason": "bad"}, concurrency=1)
...
def test_grade_and_eval_share_one_contract():
    ...
    for g, e in zip(graded, evald):
        assert set(k for k in g if k.startswith("judge")) == set(
            k for k in e if k.startswith("judge")
        )
...
def test_selection_takes_passes_but_never_destroys_failures():
    rows = [
        dict(r, reward=i % 2, prompt=f"ask {i}", scenario_id=f"s{i}")
        for i, r in enumerate(RAW_UNGRADED * 4)
    ]
    scored = ScoredData(
        [dict(r, judge_status="ok") for r in rows], run_id="t", source="grade", judge_name="j"
    )
    selected, _report = scored.select_for_sft()
    assert all(r["reward"] == 1 for r in selected)
    assert len(scored.failures()) == 6, "selection must not remove failures from the scored set"
...
def test_preference_pairs_require_same_prompt_pass_and_fail():
    rows = [
        ...
        {"prompt": "same ask", "reward": 0, "judge_status": "ok", "final_text": "bad", "steps": []},
        {
            "prompt": "lonely ask",
            "reward": 1,
            "judge_status": "ok",
...
def test_lineage_survives_scoring_selection_and_export():
    ...
    for r in sim_rows:
        r.pop("messages")
    scored = run_judge(sim_rows, lambda row: 1, judge_name="my_judge", concurrency=1)
...
def test_full_feedback_loop_offline():
    ...
    def judge(row):
        return {
            "reward": int(
                "confirmed" in str(row.get("final_text", "")).lower()
                or "$" in str(row.get("final_text", ""))
            ),
    ...
    selected, _ = scored.select_for_sft()
    exported = training_rows(selected or list(scored)[:1], system_prompt=POLICY, tools=TOOLS)
