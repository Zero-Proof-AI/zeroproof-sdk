...
POLICY = (
    "Look up an order with get_order before any refund_order call. "
    ...
    return {
        ...
        "messages": [
            {"role": "user", "content": f"where is order A{i}"},
            {
                "role": "assistant",
                "content": "",
                "tool_calls": [{"name": tool, "arguments": {"order_id": f"A{i}"}}],
            },
...
def honest_judge(t):
    return {
        "reward": int("invented" not in t["final_text"]),
        "reason": "grounded" if "invented" not in t["final_text"] else "made up a fact",
    }
...
def test_full_loop_scored_to_export_to_eval_to_traces(tmp_path):
    ...
    assert all(r["lineage"]["scoring_run_id"] == scored.run_id for r in scored)

    out = str(tmp_path / "train.jsonl")
    report = export_training(scored.passes(), out, system_prompt=POLICY, tools=TOOLS)
    ...
    rollouts += [_traj(i, final="I invented a refund confirmation.") for i in (22, 23)]
...
def test_judge_contract_edges():
    ...
    extras = normalize_judge_result(
        {"reward": 0, "failure_class": "fabrication", "confidence": 0.9}
    )
    assert extras["judge_meta"] == {"failure_class": "fabrication", "confidence": 0.9}
...
def test_preference_pairs_and_export(tmp_path):
    rows = [
        _traj(1, final="Order shipped."),
        _traj(1, final="I invented it."),
...
def test_bool_rewards_count_in_trace_mining():
    rows = [dict(_traj(1, final="fine"), reward=False), dict(_traj(2, final="fine"), reward=True)]
