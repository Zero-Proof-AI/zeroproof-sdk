...
def test_one_row_region_is_flagged_low_support():
    ...
    assert (
        abs(sum(r["budget_share"] for r in state["regions"]) + state["exploration_share"] - 1.0)
        < 0.01
    )
...
def test_turn_controller_corrects_at_half_gain():
    ...
    high = [sample_turn_budget(s, f"k{s}", 12, avg_turns=6, running_mean=10) for s in range(400)]
