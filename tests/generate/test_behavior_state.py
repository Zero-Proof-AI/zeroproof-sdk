...
def _mrow(marker, ok, version, tool="edit_file", dims=None):
    return {
        "prompt": f"p-{marker}-{version}-{ok}",
        "model_version": version,
        "scores": {marker: 1 if ok else 0},
        "scenario_dimensions": dims or {"tool": tool, "stance": "hurried"},
        "steps": [{"tool": tool, "arguments": {}, "result": {"status": "ok"}}],
        "final_text": "done",
    }
...
def test_three_round_lifecycle_with_model_versions():
    ...
    state = behavior_state(rows, targeted=["edits_before_reading", "claims_fix_without_test"])
...
def test_region_is_predicate_not_tuple_and_recipe_collects_coords():
    rows = [
        _mrow("edits_before_reading", False, "v0", dims={"tool": "edit_file", "stance": "hurried"}),
        _mrow(
            "edits_before_reading",
            False,
            "v0",
            dims={"tool": "edit_file", "stance": "ordinary", "world_state": "entity exists"},
        ),
    ]
...
def test_fault_response_and_capability_fallback():
    fault_fail = {
        ...
        "reward": 0,
        "model_version": "v0",
        "steps": [{"tool": "search", "arguments": {}, "result": {"status": "timeout"}}],
        "final_text": "gave up",
    }
    fault_ok = {
        "prompt": "b",
...
def test_rows_carry_dims_and_model_version_everywhere():
    ...
    assert all(t.get("model_version") == "v3-adapter" for t in data.trajectories)
...
def test_allocation_actually_shifts_generation():
    ...
    hot = [
        {
            "prompt": f"refund order {i} failed again",
            "steps": [
                {
                    "tool": "create_refund",
            ...
            "final_text": "The refund timed out.",
            "reward": 0,
            "model_version": "v0",
        }
        for i in range(4)
    ...
    def refund_share(data):
        n = sum(
            ...
            if (t.get("scenario_dimensions") or {}).get("tool") == "create_refund"
        )
    ...
    assert any(r["region"] == "recover_after_timeout" for r in state["regions"])
...
def test_applied_is_false_when_no_cell_ever_boosted():
    ...
    hot = [
        {
            "prompt": f"legacy lookup {i} failed",
            "steps": [{"tool": "legacy_lookup", "arguments": {}, "result": {"status": "timeout"}}],
            "final_text": "It failed.",
            "reward": 0,
            "model_version": "v0",
        }
        for i in range(4)
...
def test_ungraded_fault_rows_are_unknown_not_failed():
    ...
    rows = [
        {
            "prompt": f"lookup {i} timed out",
            "steps": [{"tool": "get_order", "arguments": {}, "result": {"status": "timeout"}}],
    ...
    state = behavior_state(rows)
    region = next(r for r in state["regions"] if r["region"] == "recover_after_timeout")
