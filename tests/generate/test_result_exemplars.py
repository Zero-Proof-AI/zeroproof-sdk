...
TRACES = [
    ...
    },
    {
        ...
        "steps": [
            {
                "tool": "lookup_order",
                "arguments": {"order_id": "130"},
                "output": json.dumps({"orders": [{"order_id": "130", "eta": "2026-09-04"}]}),
            }
        ],
...
def test_exemplars_are_mined_shape_diverse_and_filtered():
    exemplars = mine_result_exemplars(
        [
            *TRACES,
            ...
            {
                "steps": [
                    {"tool": "lookup_order", "arguments": {}, "result": {}},
                    {"tool": "lookup_order", "arguments": {}, "result": {"status": "ok"}},
                    {
...
def test_per_tool_cap_and_serialized_size_cap():
    rows = [{"steps": [{"tool": "t", "arguments": {}, "result": {f"k{i}": i}}]} for i in range(5)]
    assert len(mine_result_exemplars(rows)["t"]) == 3
    big = mine_result_exemplars(
        [{"steps": [{"tool": "t", "arguments": {}, "result": {"text": "x" * 5000}}]}]
    )
...
def test_exemplar_templates_pattern_mock_results():
    ...
    env = MockEnvironment(TOOLS, world_state="entity exists", result_shapes=shapes)
