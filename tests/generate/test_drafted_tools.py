...
def test_draft_tools_parses_fenced_json_and_marks_drafted(monkeypatch):
                ...
                [
                    {
                        ...
                        "description": "Balance of one account",
                        "parameters": {
                            "type": "object",
    ...
    tools = gen.draft_tools(
        "A personal finance assistant that confirms before moving money.", backend_spec="openai:m"
    )
    names = [t["function"]["name"] for t in tools]
    assert names == ["get_account_balance", "transfer_funds"]
    assert all(t["drafted"] is True for t in tools)
    assert tools[1]["function"]["parameters"]["required"] == [
        "from_account",
        "to_account",
        "amount",
    ]
...
def test_draft_tools_needs_two_valid_tools(monkeypatch):
    monkeypatch.setattr(
        ...
        "complete",
        lambda *a, **k: {
            "content": json.dumps([{"name": "only_one", "parameters": {}}]),
            "tool_calls": [],
        },
    )
