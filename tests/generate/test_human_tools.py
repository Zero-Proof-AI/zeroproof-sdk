...
def test_human_tool_answer_comes_from_the_user_voice(monkeypatch):
                ...
                "tool_calls": [
                    {
                        ...
                        "function": {
                            "name": "ask_user",
                            "arguments": json.dumps({"question": "Delete branch old-x for real?"}),
                        },
                    }
                ],
            }
    ...
    runner = zagents.local_model(
        ...
        system="Confirm before destructive actions.",
        max_turns=3,
    )
    ...
    assert ask_steps[0]["result"] == {"answer": "yes, delete it, we shipped that"}
    # the answering call put the model in the person's seat, not the assistant's
    assert any(
        m.get("role") == "system" and "person the assistant is helping" in str(m.get("content"))
        for m in calls[1]
    )
...
def test_human_answer_carries_the_situation_stance(monkeypatch):
    ...
    assert any(
        m.get("role") == "system" and "wrong about a detail" in str(m.get("content"))
        for m in calls[1]
    )
