...
def test_amplifies_to_target_with_dedup(monkeypatch):
    ...
    def fake_complete(url, model, messages, **kw):
        ...
        batch = [f"tell me about yourself v{len(calls)}-{i}" for i in range(25)]
...
def test_amplify_is_opt_in_and_respects_offline(monkeypatch):
    ...
    simulate(
        situations=6,
        **dict(common, advanced=dict(common["advanced"], seed_prompts=["list my recent expenses"])),
    )
    simulate(seeds=["list my recent expenses"], situations=6, **common)
    assert generator.amplify_seeds(["你是谁？", "谁造的你？"], 2) == ["你是谁？", "谁造的你？"]
