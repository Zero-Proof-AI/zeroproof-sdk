...
def _run(weight: float):
    return simulate_from_traces(
        TRACES, scripted_agent, tools=TOOLS, policy=POLICY, steering_weight=weight, **_OFFLINE
    )
...
def test_draw_helper_biases_toward_the_aimed_pool():
    ...
    background = [region for region in ranked if not _aimed(region, front)][:6]
    ...
    assert n_aimed > sum(1 for region in background if _aimed(region, front))
...
def test_w0_and_w1_produce_different_axis_distributions():
    ...
    def tool_counts(data) -> Counter:
        return Counter(
            str((t.get("scenario_dimensions") or {}).get("tool"))
    ...
    def mined_share(counts: Counter) -> float:
        ...
        return sum(n for tool, n in counts.items() if tool in mined_tools) / total

    assert mined_share(c1) > mined_share(c0), "w=1 must overrepresent the trace-mined axis values"

    targeted = [t for t in d1.trajectories if (t.get("steering") or {}).get("origin") == "targeted"]
...
def test_w0_marks_zero_targeted_rows():
    data = _run(0.0)
    assert all((t.get("steering") or {}).get("origin") != "targeted" for t in data.trajectories)
...
def test_metadata_reports_only_what_applied():
    ...
    marked = sum(
        1 for t in d1.trajectories if (t.get("steering") or {}).get("origin") == "targeted"
    )
