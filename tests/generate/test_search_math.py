...
def _rate(novelty: float, temperature: float = 1.0, n: int = 2000) -> float:
    rng = random.Random(0)
    return (
        sum(accept_anneal_candidate(novelty, temperature=temperature, rng=rng) for _ in range(n))
        / n
    )
...
def test_explore_swaps_in_the_most_novel_pool_members():
    ...
    out = apply_annealing_explore(
        [0, 1], texts_len=5, novelty_of=novelty_of, need=2, round_index=0, seed=0
    )
...
def test_idle_arm_carries_no_vote():
    ...
    for _ in range(10):
        weights = reallocate_search_arms(weights, {"structured": 0.5, "llm_guided": 0.5})
...
def test_arm_that_found_more_gains_share():
    weights = dict(SEARCH_ARMS)
    for _ in range(5):
        weights = reallocate_search_arms(weights, {"structured": 1.5, "llm_guided": 0.1})
