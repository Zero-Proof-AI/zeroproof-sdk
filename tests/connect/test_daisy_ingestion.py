...
from tests.helpers import FakeWriter, scripted_agent
...
DAISY_POLICY = (
    "You are daisy, a coding agent. Only act on files the user "
...
def _span(trace, sid, parent, name, start_ms, attrs, events=(), status_code=1):
    return {
        "traceId": trace,
        "spanId": sid,
        "parentSpanId": parent,
    ...
    spans = [
        _span(
            ...
            [
                ...
                _attr("gen_ai.input.messages", "fix the failing test in utils.py"),
                _attr(
                    "gen_ai.output.messages",
                    json.dumps(
                        [{"role": "assistant", "content": "The command failed, so I stopped."}]
                    ),
                ),
            ],
        ),
        _span(
            ...
            [
                ...
                _attr(
                    ...
                    json.dumps([{"role": "user", "content": "fix the failing test in utils.py"}]),
                ),
                _attr(
                    "gen_ai.output.messages",
                    json.dumps(
        ...
        _span(
            ...
            [
                ...
                _attr("gen_ai.tool.call.arguments", json.dumps({"path": "utils.py"})),
        ...
        _span(
            ...
            [
                ...
                _attr("gen_ai.tool.call.arguments", json.dumps({"command": "pytest -q"})),
            ...
            events=[
                {
                    "name": "exception",
                    "time": "0",
                    "attributes": [_attr("exception.message", "pytest exited 1: 3 failed")],
                }
            ],
            status_code=2,
        ),
...
def test_daisy_bundle_converts_with_error_status_surfaced():
    ...
    assert ok == "def add(a, b): return a - b", "success results must pass through untouched"
...
def test_daisy_failure_reaches_mining_and_report():
    ...
    assert any("error" in k for k in report["faults_observed"]), report["faults_observed"]
...
def test_platform_gate_spans_order_by_started_ms():
    ...
    base = 1787841777000
    tool_seq = [
        ("list_files", "{}", '{"status": "ok", "files": ["a.py"]}'),
    ...
    assert [s["tool"] for s in row["steps"] if "tool" in s] == [t for t, _, _ in tool_seq]
    assert row["reward"] == 1
