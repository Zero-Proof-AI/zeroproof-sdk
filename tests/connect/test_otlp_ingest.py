"""zeroproof.ingest: the exporter env, the nanosecond guard, gzip, the dataset override."""
...
def test_otel_env_points_the_exporter_at_the_gate_with_the_key_as_a_header(monkeypatch):
    monkeypatch.delenv("ZEROPROOF_TRACE_URL", raising=False)
    ...
    assert env["OTEL_RESOURCE_ATTRIBUTES"] == "zeroproof.dataset=prod-refunds"
...
def test_otel_env_honors_the_base_url_override_and_the_env_var(monkeypatch):
    ...
    monkeypatch.setenv("ZEROPROOF_TRACE_URL", "http://gate.test")
...
def test_millisecond_timestamps_are_refused_before_anything_is_uploaded(monkeypatch):
    ...
    with pytest.raises(ingest.ZeroProofIngestError, match="not nanoseconds"):
        ingest.send_traces("zp_abc", body)
...
def test_gate_rejection_becomes_an_ingest_error_with_the_status(monkeypatch):
    ...
    with pytest.raises(ingest.ZeroProofIngestError, match="HTTP 415 protobuf not accepted"):
...
def test_ingest_traces_renames_the_dataset_on_every_resource(tmp_path, monkeypatch):
    ...
    batch["resourceSpans"][0]["resource"]["attributes"].append(
        {"key": "zeroproof.dataset", "value": {"stringValue": "old-name"}}
    )
    ...
    names = [a["value"]["stringValue"] for a in attrs if a["key"] == "zeroproof.dataset"]
...
def test_ingest_traces_refuses_an_empty_file(tmp_path, monkeypatch):
    ...
    path.write_bytes(b"")
    with pytest.raises(ingest.ZeroProofIngestError, match="empty file"):
        ingest.ingest_traces("zp_abc", str(path))
...
def test_list_traces_reads_with_the_key_and_surfaces_a_rejection(monkeypatch):
    ...
    with pytest.raises(ingest.ZeroProofIngestError, match="HTTP 401"):
