...
- `zps.HackMonitor(run, holdout=, proxy=, gold=, ...)`: is the run
  ...
  `--stop-on`).
- `run_judge(rows, judge, scale=(lo, hi))`, `evaluate(..., scale=)` and
  `data.grade(judge=, scale=)` (rlhf-book ch. 11, ratings vs rankings): a
  Likert or 0-to-10 judge is read as a rating on that scale, `reward` is
  the rating mapped onto [0, 1] (exact 0 / 1 at the ends, so binary
  selectors still work), and `judge_meta` keeps `rating` and `scale`. A
  rating outside the scale is `invalid_result` with `rating_out_of_scale`;
  without a scale nothing changes.
