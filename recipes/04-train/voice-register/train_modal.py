...
TRAIN_IMAGE = (
    ...
    .pip_install(
        "torch==2.7.1", "transformers==4.54.0", "trl==0.19.1", "peft==0.16.0",
        "datasets==3.6.0", "accelerate==1.8.1", "huggingface_hub>=0.34", "requests>=2.25",
    )
...
@app.function(image=TRAIN_IMAGE, gpu="H100", timeout=2 * 60 * 60,
              volumes={VOL: adapters, "/root/.cache/huggingface": hf_cache}, secrets=secrets)
def train(job: dict, rows: list[dict]) -> dict:
    ...
    lora = LoraConfig(r=int(job.get("rank", 16)), lora_alpha=int(job.get("rank", 16)) * 2,
                      lora_dropout=0.0, bias="none", task_type="CAUSAL_LM",
                      target_modules=["q_proj", "k_proj", "v_proj", "o_proj",
                                      "gate_proj", "up_proj", "down_proj"])
    adapter_dir = f"{VOL}/{name}/adapter"
    cfg = SFTConfig(output_dir=f"{VOL}/{name}/ckpt",
                    num_train_epochs=float(job.get("epochs", 2.0)),
                    learning_rate=float(job.get("lr", 1e-4)),
                    per_device_train_batch_size=2, gradient_accumulation_steps=4,
                    bf16=True, packing=False, max_length=int(job.get("max_len", 3072)),
                    logging_steps=10, save_strategy="no", report_to=[], seed=17)
    tr = SFTTrainer(model=model, args=cfg, train_dataset=Dataset.from_list(rows),
                    processing_class=tok, peft_config=lora)
    tr.train()
    tr.save_model(adapter_dir)
    tok.save_pretrained(adapter_dir)
    adapters.commit()
    return {"adapter": adapter_dir, "rows": len(rows),
            "loss": [h for h in tr.state.log_history if "loss" in h][-3:]}


@app.function(image=SERVE_IMAGE, gpu="H100", timeout=2 * 60 * 60,
              volumes={VOL: adapters, "/root/.cache/huggingface": hf_cache}, secrets=secrets)
def evaluate(job: dict, prompts: list[dict]) -> dict:
    ...
    cmd = [sys.executable, "-m", "vllm.entrypoints.openai.api_server",
           "--model", base, "--served-model-name", "base", "--port", "8000",
           "--api-key", key, "--max-model-len", "16384",
           "--gpu-memory-utilization", "0.85", "--disable-log-requests",
           "--enable-lora", "--max-lora-rank", str(int(job.get("rank", 16))),
           "--lora-modules", f"tuned={adapter}"]
    proc = subprocess.Popen(cmd)
    url = "http://127.0.0.1:8000/v1"
    for _ in range(180):
        try:
            if requests.get(f"{url}/models", headers={"Authorization": f"Bearer {key}"}, timeout=5).ok:
                break
        except Exception:
            pass
        time.sleep(5)
    ...
    def ask(model_name: str, p: dict) -> dict:
        body = {"model": model_name,
                ...
                "temperature": 0.0, "max_tokens": int(job.get("max_new_tokens", 2048))}
        try:
            r = requests.post(f"{url}/chat/completions", json=body,
        ...
        return {"ask": p["ask"], "required": p.get("required", []),
                "probe": p.get("probe", False), "reply": txt, "model": model_name,
    ...
    try:
        for m in ("base", "tuned"):
            with ThreadPoolExecutor(max_workers=12) as pool:
                out[m] = list(pool.map(lambda p: ask(m, p), prompts))
    finally:
        proc.terminate()
        try:
...
@app.function(image=TRAIN_IMAGE, timeout=30 * 60,
              volumes={VOL: adapters, "/root/.cache/huggingface": hf_cache}, secrets=secrets)
def publish(job: dict, card: str) -> dict:
    ...
    api.upload_folder(folder_path=d, repo_id=repo, repo_type="model",
                      commit_message="Concise register adapter")
...
def main(job: str, steps: str = "train,eval", out: str = ""):
    ...
    if "train" in want:
        rows = [json.loads(l) for l in (base_dir / cfg["rows_file"]).read_text().splitlines() if l.strip()]
    ...
    if "eval" in want:
        ...
        d = Path(out or base_dir); d.mkdir(parents=True, exist_ok=True)
