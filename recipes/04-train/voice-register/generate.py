...
try:
    ...
    pass
import prompts as VP, voice as VC
from concurrent.futures import ThreadPoolExecutor

OUT=os.environ.get("VOICE_OUT","out/voice"); os.makedirs(OUT, exist_ok=True)
N_TRAIN=int(os.environ.get("VN_TRAIN","520")); N_HOLD=int(os.environ.get("VN_HOLD","170"))
...
print(f"prompts: train {len(train_p)}, holdout {len(hold_p)} | user overlap "
      f"{len({p['user_id'] for p in train_p} & {p['user_id'] for p in hold_p})}", flush=True)
...
def plain(p):
    try:
        return VC.RA.chat([{"role":"system","content":p["system"]},{"role":"user","content":p["ask"]}],
                          temperature=0.6, max_tokens=400).strip()
    except Exception:
        return ""
...
vr=sum(v)/len(v); cr=sum(c)/max(1,len(c))
...
print(f"  separation {vr-cr:+.3f}  {'PASS' if vr-cr>0.5 else 'FAIL: the data cannot teach the register'}")

print(f"\nGATE B  length, the confound to watch")
lv=[len(r["reply"]) for r in sample]; lc=[len(x) for x in controls]
print(f"  voice rows median {statistics.median(lv):.0f} chars | controls median {statistics.median(lc):.0f}")
