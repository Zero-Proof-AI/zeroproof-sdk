...
| [`voice-register`](04-train/voice-register) | a speaking register trained into the weights: a two-sided reward (judge for the register, program for the identifiers the answer must keep), two gates that say in minutes whether the data can teach it, greedy fixed-prompt eval of both arms in one vLLM process, `finish_reason` per row, persona-strip probes, a paired interval and verdict | an OpenAI-compatible endpoint and key; Modal and an H100 to train; `smoke.sh` needs nothing | minutes to generate, about half an hour to train and eval |
