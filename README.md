...
**Before and after.** `delta_report` runs `compare_runs` on pass@1 and every marker both row sets share. `target=` names the metric the training was meant to move and gives the headline; `must_not_regress=` names the behaviors whose significant drop fails the report; any other significant drop is a warning. `format_delta_report(report)` prints one line per metric. `eval_variance(run_1, run_2, run_3)` is the eval's own re-run standard deviation (three or more evaluations of the same model): `run_std` for pass@1 and `run_std_by_metric` with one floor for pass@1 and each marker the runs share. Pass the mapping as `run_std=` and every metric is judged against its own floor; a delta inside `noise_band(floor)` = `1.96 x floor x sqrt(1/n_a + 1/n_b)` (one run per side: `1.96 x sqrt(2) x floor`, since a delta is the difference of two re-run draws; the t quantile instead when the report pooled the floor from `runs=3` on both sides) is `within_noise`, and a target there reads `within_eval_noise` rather than moved, since re-running the eval moves it that much on its own (rlhf-book ch. 16, appendix C). `noise_rule` on the report spells the band out. Pass the mapping, not the scalar: a marker that applies to a subset of tasks is several times noisier than pass@1, which averages over all of them, and pass@1's band reads a model compared to itself as a regression (#300). A metric the mapping has no floor for is never judged against another's; it gets `noise_note: no_replicate_floor` and a warning. `by=` names a row key, a marker, or a callable that groups rows (a prompt category, a tool, a persona); the report then carries `groups`, the target compared within each group, and `groups_down` for any group whose target dropped significantly while the headline moved. A headline over one dominant kind of prompt cannot hide the other kinds that way.
