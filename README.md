...
**Before and after.** `delta_report` runs `compare_runs` on pass@1 and every marker both row sets share. `target=` names the metric the training was meant to move and gives the headline; `must_not_regress=` names the behaviors whose significant drop fails the report; any other significant drop is a warning. `format_delta_report(report)` prints one line per metric. `eval_variance(run_1, run_2, run_3)` is the eval's own re-run standard deviation (three or more evaluations of the same model); passing it as `run_std=` makes any delta inside `noise_band(run_std)` = `1.96 x run_std x sqrt(1/n_a + 1/n_b)` `within_noise`, and a target there reads `within_eval_noise` rather than moved, since re-running the eval moves it that much on its own (rlhf-book ch. 16, appendix C). `n_a` and `n_b` are the runs each side's mean averages over (one each in a plain before/after, so `sqrt(2)` times `run_std`: a delta is the difference of two re-run draws); when the report works `run_std` out from `runs=3` on both sides it uses the t quantile for those runs instead (2.78 at df=4), and `noise_rule` spells the band out. A flat `2 * run_std` let about 15% of pure-noise deltas through instead of 5%. `by=` names a row key, a marker, or a callable that groups rows (a prompt category, a tool, a persona); the report then carries `groups`, the target compared within each group, and `groups_down` for any group whose target dropped significantly while the headline moved. A headline over one dominant kind of prompt cannot hide the other kinds that way.
