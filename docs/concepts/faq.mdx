...
**Mid-training or post-training?** Both. Mid-training continues training
on your domain data so the model knows your world. Post-training with SFT,
DPO or GRPO teaches it how to behave. Most teams start with post-training.
...
want in a sentence. The SDK drafts the policy and the simulated rollout
set from it, so there is training data before there is traffic.
