...
data = wai.simulate(
    wai.seeded_agent([get_order]),  # a stand-in agent that misbehaves on a labeled share of rows
    tools=[get_order],
...
scored = data.grade(judge=lambda row: {"reward": int(not row["seeded"])})  # any judge or verifier
print(scored.pass_at)
rows = scored.select(mode="rl")  # leaked answer keys dropped, the 20 to 80% band, unanimous groups dropped, hack scan
...
program is the same. The package then does the bookkeeping that is easy
to skip and expensive to get wrong: pass rates with intervals, difficulty
bands for RL, a check that your judge agrees with people, decontamination
against your eval set, and a scan for rewards the policy can game.
