...
```python
wai.configure(agent=wai.OpenAI("gpt-4.1-mini"))  # a Backend goes through configure
data = wai.simulate(tools=TOOLS, system_prompt=POLICY)
```

Leave `agent=` out and the run uses what `configure` set, else the model
While hosts on the key from `whileai login`. Useful for a policy that does
