...
method comes from, is the [README](../README.md).
...
Have an agent and want a pass rate with an interval? Start at [docs/evals.md](evals.md) (offline, seconds, `coverage_gap` names what your tests miss).
...
[docs/evals.md](evals.md); the runnable version is
...
| Select | [`recipes/03-select/character`](../recipes/03-select/character) | Character training from a constitution: the OpenAI Model Spec's style traits become graded rows, preference pairs and SFT rows, with the judge checked against the spec's own labels and a before/after measurement. Offline by default. How-to: [docs/character-training.md](character-training.md). |
