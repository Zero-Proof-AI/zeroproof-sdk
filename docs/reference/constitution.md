...
description: "What whileai is, what we believe, and where each belief is enforced in the code. Repeatable science, replicated papers as proof, your keys, ergonomics as the product."
---

<Note>
This page mirrors [`CONSTITUTION.md`](https://github.com/whilehq/whileai-sdk/blob/main/CONSTITUTION.md) at the repository root, which is the canonical copy; a test keeps the two identical. The three agents that apply it on demand live in [`.claude/agents/`](https://github.com/whilehq/whileai-sdk/tree/main/.claude/agents): `style-guide` (the SDK surface against the [style guide](/reference/style) and the book), `docs-designer` (these docs and the website, with a designer's page contract), `user-sim` (a researcher running a recipe on their own keys and compute, filing what got in the way).
</Note>
