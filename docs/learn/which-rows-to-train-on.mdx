...
sft selection: kept 16 of 64 rows
  ...
  completions per prompt: mean 4.0, median 4, max 4; 0 of 16 prompts have one. Rejection-sampling selection wants 10 to 30 so the pick is not biased (rlhfbook.com/c/10-rejection-sampling.html; Llama 3 samples 10 to 30). Raise repeats= if you mean to choose among completions rather than filter.
